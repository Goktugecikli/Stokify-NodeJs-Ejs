  ...
  async JoinCompanyByInvateCode(invateCode, userName) {
    try {
      ...
      if (!companyDetailsResult  || companyDetailsResult.length === 0) {
        console.log("Burada");
        return { success: false, message: "Wrong CompanyId" };
      }
      ...
      console.log(JSON.stringify(companyDetailsResult));
      console.log(JSON.stringify(userDetailsResult));


      let joinResult = await this.userRepository.JoinCompanyByUserIdAndCompanyId(userDetailsResult[0].UserId, companyDetailsResult[0].CompanyId);
