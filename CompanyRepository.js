...
class CompanyRepository {
  ...
  async AddProductToCompanyByUserNameAndProduct_id(product_id, userName) {
    try {
      await this.dbManager.connectDB(); // Veritabanına bağlan
      let params = { id: product_id, userName: userName };
      let query =
        "INSERT INTO CompanyProducts (ProductId, CompanyId) VALUES ((select ProductId from Products where Id=@id),(Select * from CompanyUsers where UserId = (Select UserId from Users where UserName=@userName)));SELECT SCOPE_IDENTITY() AS Id;";
      return await this.dbManager.queryWithResult(query, params);
    } catch (err) {
      // console.error("Error repossitory connection. Error ", err);
    } finally {
